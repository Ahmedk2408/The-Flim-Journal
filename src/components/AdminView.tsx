...
interface AdminViewProps {
  ...
  onUpsertPost?: (post: Post) => Promise<void>;
...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const handlePostSubmit = (e: React.FormEvent) => {
    ...
    // Use per-post upsert (Supabase) if available, else fall back to bulk
    if (onUpsertPost) {
      const savedPost = postFormMode === 'add' ? updatedPosts[0] : updatedPosts.find(p => p.id === editingPost?.id) || updatedPosts[0];
      onUpsertPost(savedPost);
    } else {
  ...
  const toggleFeaturedState = (post: Post) => {
    ...
    if (onUpsertPost) {
      const toggled = updated.find(p => p.id === post.id);
      if (toggled) onUpsertPost(toggled);
    } else {
